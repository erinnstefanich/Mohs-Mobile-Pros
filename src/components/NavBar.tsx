...
export default function NavBar() {
  ...
  return (
    <header className={isHome ? 'relative z-40 border-b border-slate-100 bg-white shadow-[0_8px_24px_rgba(11,35,69,0.05)]' : 'sticky top-0 z-40 border-b border-slate-200/80 bg-white shadow-[0_10px_30px_rgba(11,35,69,0.06)]'}>
      <div className={isHome ? 'relative mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8' : 'container relative'}>
        <TopBar compact={isHome} />
        <nav className={isHome ? 'flex min-h-[88px] items-center justify-between gap-2 py-1 sm:gap-4 min-[900px]:min-h-[87px] min-[900px]:items-end min-[900px]:gap-5 min-[900px]:pb-[14px] xl:min-h-[108px] xl:pb-4' : 'flex min-h-[112px] items-center justify-between gap-2 py-1 sm:gap-4 lg:min-h-[124px] lg:items-end lg:gap-5 lg:pb-3'}>
          <Logo compact={isHome} />

          <div className={isHome ? 'hidden min-w-0 flex-1 items-center justify-end gap-[13px] pb-[1px] min-[900px]:flex xl:gap-5' : 'hidden min-w-0 flex-1 items-center justify-end gap-2 pb-2 lg:flex xl:gap-3'}>
            {headerLinks.map((link) => (
              <Link key={link.href} href={link.href} prefetch={false} aria-current={isActive(link.href) ? 'page' : undefined} className={isHome ? 'group relative whitespace-nowrap py-2 text-[11px] font-extrabold text-brand-navy transition duration-200 hover:text-brand-orange xl:text-[13px]' : 'group relative py-2 text-xs font-extrabold text-brand-navy transition duration-200 hover:text-brand-orange xl:text-sm'}>
                {link.label}
            ...
            <Link href="/request-service" prefetch={false} aria-current={pathname === '/request-service' ? 'page' : undefined} className={isHome ? 'btn-primary min-h-[36px] min-w-[103px] whitespace-nowrap rounded px-3 py-2 text-[10px] xl:min-h-11 xl:px-5 xl:text-xs' : 'btn-primary min-h-10 px-4 py-2 text-xs xl:px-5'}>Request Service</Link>
          </div>
